...
class IssuesModelForm(BootstrapForm, forms.ModelForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        # 1.获取当前项目的所有问题类型 [(1,'xx'),(2,"xx")], choices表示前面字段可选只能是否面筛选出来的
        self.fields['issues_type'].choices = models.IssuesType.objects.filter(
            project=request.redmine.project).values_list('id', 'title')

        # 2.获取当前项目的所有模块
        module_list = [("", "没有选中任何项"), ]
        module_object_list = models.Module.objects.filter(project=request.redmine.project).values_list('id', 'title')
        module_list.extend(module_object_list)
        self.fields['module'].choices = module_list

        # 3.指派和关注者
        # 数据库找到当前项目的参与者 和 创建者
        total_user_list = [(request.redmine.project.creator_id, request.redmine.project.creator.username), ]    # 这就是创建者
        project_user_list = models.ProjectUser.objects.filter(project=request.redmine.project).values_list('user_id', 'user__username') # 参与者
        ...
        # 4. 当前项目已创建的问题（父问题，做issue关联）
        parent_list = [("", "没有选中任何项")]
        parent_object_list = models.Issues.objects.filter(project=request.redmine.project).values_list('id', 'subject')
