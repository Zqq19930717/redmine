...
class UserInfo(models.Model):
    # 加了 db_index=True 数据库会创建索引, 以后根据有索引的字段进行数据库查询速度就会很快
    username = models.CharField(verbose_name="用户名", max_length=32, db_index=True)
    # 这里的EmailField写进数据库里其实和CharField一样，但是和ModelForm结合起来使用时, 会匹配邮箱的正则表达式, 不符合邮箱格式modelform不给验证通过
    ...
    # 用户注册后null=True, blank=True默认价格策略为空(免费版), 加了这个字段性能会更快，因为不用按照id排序取最大交易id，而是直接根据user和price_policy去PricePolicy去取
...
class Module(models.Model):
    """ 模块（里程碑）"""
    MODULE_INIT_LIST = ["UI", 'FOTA', 'WIFI', '功耗', 'CP', 'AP', 'Tools']
...
class ProjectInvite(models.Model):
    ...
    count = models.PositiveIntegerField(verbose_name='可邀请人数', null=True, blank=True, help_text='空表示无数量限制')
